							...
							<ul class="dropdown-menu dropdown-user">
								<li>
									<div class="user-box">
										...
										<div class="u-text">
											<h4>李坤</h4>
											<p class="text-muted">2581437060@qq.com.com</p><a href="profile.html" class="btn btn-rounded btn-danger btn-sm">修改 密码</a></div>
										</div>
									</li>
                                    ...
                                    <c:forEach var="list" items="${findApparatusList}">
                                        <tr>
                                        	...
                                        	<td>${list.aname}</td>
                                        		...
                                        		<c:if test="${list.astate==0}"><td>正常</td></c:if>
												<c:if test="${list.astate==1}"><td>损坏</td></c:if>
											
                                        	<td>
                                        	<c:if test="${list.astate==1}">
                                        	<button class="btn btn-info btn-fill " onclick="update(${list.id})">维修</button></c:if>
                                        	<c:if test="${list.astate==0}">
                                        	<button class="btn btn-warning btn-fill " onclick="update1(${list.id})">已坏</button></c:if>
...
function update(id){
	$.ajax({
		...
		data: {id : id,type:1},
		success: function(data){
			layer.msg('维修成功!',{icon: 1,time:2000});
			setTimeout(function(){
				window.location.href='apparatusTable';
			},2000)
		},
...
function update1(id){
	$.ajax({
		...
		data: {id : id,type:0},
		success: function(data){
			layer.msg('确认已坏!',{icon: 1,time:2000});
			setTimeout(function(){
				window.location.href='apparatusTable';
			},2000)
		},
